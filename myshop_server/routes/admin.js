...
router.get('/changeStatus', async (ctx) => {
    ...
    let data = await ctx.model[collectionName].find({ "_id": id });
    console.log(data);
    if (data.length > 0) {
        if (data[0][attr] == 1) {
            let json = { /*es6 属性名表达式*/
...
router.get('/del', async function (ctx) {
    let collection = ctx.query.collection; /*数据库表*/
    let _id = ctx.query._id;   /*删除 id*/
    let delResult = await ctx.model[collection].deleteOne({ "_id": _id });
...
let managerRouter = require('./admin/manager.js');
let loginRouter = require('./admin/login.js');
let goodsCateRouter = require('./admin/goodsCate.js');
let goodsRouter = require('./admin/goods.js');
let orderRouter = require('./admin/order.js');
let focusRouter = require('./admin/focus.js');
